...
def run_query():
    ...
    if not all([host, user, password, database, port]):
        ...
        return jsonify({"message": "Incomplete database credentials provided."}), 400

    conn = None
    cursor = None
    try:
        print(f"DEBUG: Attempting to connect to DB: host={host}, port={port}, user={user}, database={database}")
